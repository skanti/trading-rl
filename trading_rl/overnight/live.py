...
DAILY_BAR_ANNO = BAR_EPOCH
DAILY_BAR_COLUMNS = len(BAR_COLUMNS["1Day"])
...
class AlpacaClient:
    ...
    def historical_daily_bars(
        ...
        def request_page(params: dict[str, object]) -> Mapping[str, object]:
            return dict(
                self._request(
                    "GET",
                    self.data_url,
...
def _daily_bars_to_array(
    ...
    """Encode split-adjusted Alpaca daily bars through the shared schema."""
    return encode_alpaca_bars(bars, symbol, "1Day")
...
def _merge_daily_arrays(base: np.ndarray, update: np.ndarray) -> np.ndarray | None:
    ...
    return merge_bar_arrays(base, update, expected_columns=DAILY_BAR_COLUMNS)


def _atomic_save_array(path: Path, array: np.ndarray) -> None:
    atomic_save_bar_array(path, array, expected_columns=DAILY_BAR_COLUMNS)
