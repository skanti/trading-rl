...
# Editable checkouts keep the established, user-editable path as the source of
# truth. A wheel has no repository-level compatibility file, so it uses the
# packaged copy instead.
DEFAULT_LIVE_CONFIG_PATH = (
    _REPOSITORY_LIVE_CONFIG_PATH
    if _REPOSITORY_LIVE_CONFIG_PATH.is_file()
    else PACKAGED_LIVE_CONFIG_PATH
)
